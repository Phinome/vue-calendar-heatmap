    ...
    <g class="vch__year__wrapper" :transform="yearWrapperTransform">
      ...
      >
        <rect
          ...
          v-for="(day, dayIndex) in getWeek(week, now)"
          :key="dayIndex"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      heatmap() {
        console.log(new Heatmap(this.values, this.max));
      ...
      legendWrapperTransform() {
        return {
          horizontal: `translate(${this.width[this.position] - this.SQUARE_SIZE * this.rangeColor.length - 30}, ${
            this.heigth[this.position] - this.BOTTOM_SECTION_HEIGTH
          })`,
          vertical: `translate(${this.LEFT_SECTION_WIDTH + this.SQUARE_SIZE * DAYS_IN_WEEK}, ${
            this.TOP_SECTION_HEIGTH
          })`,
        };
      },
    ...
    methods: {
      handleResize({ width, heigth }) {
        this.$emit('on-resize', { width, heigth });
      },
      tooltipOptions(day) {
        if (this.tooltip) {
          if (day.count != null) {
      ...
      getWeek(week, now) {
        return week.filter((w) => w.date < now);
      },
